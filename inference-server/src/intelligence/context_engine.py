"""
ContextEngine - Build relevance-ranked context pyramids using clean manager dependencies.
...
connection issues. Provides clean, reliable context building.
"""
...
from ..database.file_manager import FileManager, file_manager as global_file_manager
...
class ContextEngine:
    ...
    def __init__(self, hybrid_store: HybridStore, embedder: Embedder, file_manager: FileManager = None):
        ...
        self.files = file_manager or global_file_manager
    ...
    async def _get_current_note_context(self, note_path: str) -> Optional[ContextItem]:
        ...
        if not self.files:
            logger.warning("No file manager available for current note context")
            return None
    ...
    async def _get_mentioned_file_context(self, file_path: str) -> Optional[ContextItem]:
        ...
        if not self.files:
            logger.warning("No file manager available for mentioned file context")
            return None
    ...
    async def _get_similar_notes_context(self, query: str) -> List[ContextItem]:
        ...
        try:
            # Use enhanced search with document expansion for better coverage
            results = self.store.search_with_document_expansion(
                query=query,
                k=16,  # Increased from 8 to get more document coverage
                expansion_chunks=6,  # Add up to 6 neighboring chunks per document
        ...
        # Sort by relevance - ensure score is always a float
        def get_score(item):
    ...
    async def _get_recent_notes_context(self) -> List[ContextItem]:
        ...
        if not self.files:
            logger.warning("No file manager available for recent notes context")
            return items
    ...
    def get_context_sources(self, pyramid: ContextPyramid) -> List[str]:
        """Extract source paths from context pyramid."""
