...
import { ApiVaultQAChainRunner } from "@/LLMProviders/chainRunner/ApiVaultQAChainRunner";
import { logError, logInfo } from "@/logger";
import { HybridRetriever } from "@/search/hybridRetriever";
import VectorStoreManager from "@/search/vectorStoreManager";
import { ApiVectorStoreManager } from "@/search/ApiVectorStoreManager";
...
export default class ChainManager {
  ...
  public apiVectorStoreManager: ApiVectorStoreManager;
  ...
  constructor(app: App, vectorStoreManager: VectorStoreManager) {
    ...
    this.apiVectorStoreManager = new ApiVectorStoreManager();
  ...
  /**
   * Get the appropriate vector store manager based on settings
   */
  public getCurrentVectorStoreManager() {
    const settings = getSettings();
    return settings.useApiServer ? this.apiVectorStoreManager : this.vectorStoreManager;
  }
  ...
  private getChainRunner(): ChainRunner {
    ...
    switch (chainType) {
      ...
      case ChainType.VAULT_QA_CHAIN:
        // Use API-based chain runner if server mode is enabled, otherwise use original
        if (settings.useApiServer) {
          return new ApiVaultQAChainRunner(this);
        }
