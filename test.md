...
- 🚀 Router logs: "Starting vLLM server for Qwen3-30B..."
...
**Expected output:** `"qwen3-30b"` or similar local model name
...
**Expected:** Vision server auto-starts (3-5 min first time), returns `"qwen2.5-vl"`
...
**Problem:** `Model not found at ./models/Qwen3-30B-A3B-bnb-4bit`
...
curl -X POST http://localhost:8000/v1/chat/completions \
  -d '{"model":"qwen3-30b","messages":[{"role":"user","content":"text"}]}'
...
for i in {1..5}; do
  curl -X POST http://localhost:8000/v1/chat/completions \
    -d '{"model":"qwen3-30b","messages":[{"role":"user","content":"Request '$i'"}]}' &
done
